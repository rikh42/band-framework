...
class AbstractType
{
    ...
    protected function readProperty($object, $property, $default)
    {
        ...
        if (is_object($object)) {
            ...
            } elseif ($reflection->hasMethod($is)) {
    ...
    protected function writeProperty($object, $property, $value)
    {
        ...
        if (is_object($object)) {
            ...
            if (($reflection->hasMethod($set)) && ($reflection->getMethod($set)->isPublic())) {
                // call the getter to read the value (eg $object->getUsername())
                $object->$set($value);
            } elseif (($reflection->hasProperty($property)) && ($reflection->getProperty($property)->isPublic())) {
                // The object has a public property of the right name, so just use that
                ...
                // need this to support magic getters and setters
                $object->$property = $value;
            } elseif ($reflection->getShortName() == 'stdClass') {
